...
class LegacyAuditFilterTest {
    ...
    void shouldNotBlockRequest_regardlessOfApiKeyConfiguration() throws Exception {
        // ☢️ LegacyAuditFilter is audit-only by design (see TRIAGE.md S7: no Spring
        // Security on the classpath) — it logs every request but never rejects one,
        // regardless of method or whether an API key happens to be configured.
        filter = new LegacyAuditFilter(new PlantSecurityProperties("configured-key"));
        var request = mock(HttpServletRequest.class);
        var response = mock(HttpServletResponse.class);
